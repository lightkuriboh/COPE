...
namespace Judger
{
	...
	{		
		...
		private bool Compare_with(ref string ProgramOutput, string Jury_output) {
			try {
				...
				try {
					...
					Console.WriteLine ("Error reading jury's output: {0}", ex.Message);
				}
				Jury.Close();
				...
				try {
					foreach (string s in string_to_remove) {
						...
						ProgramOutput = ProgramOutput.Replace (s, " ");
					}
					//for (int i = Jury_line.Length - 1; Jury_line[i] == ' ';) Jury_line = Jury_line.Remove(i);
				} catch (Exception ex) {
		...
		private double Judge_With_Extend_Judging_Program (string path_to_program, string inpp, string outt, string Destination) {
			try {
				...
				try {
					...
					//Console.WriteLine("Judged with extend judger: {0}", final);
					return final;
				} catch {
					return 0.0;
				}
		...
		public void Process(string s, string Destination) {
			try {
				//------------------------------------------------------------
				...
				try {
					...
					Program.Compile_with (extend, Destination);
					//return;
				} catch (Exception ex) {
				...
				for (int i = 0; i < sz; i++) {
					...
					int RunningResult = 0;
					RunningResult = Program.Run_participant_code_with (inpp, TimeLimit, MemoryLimit, Destination);
